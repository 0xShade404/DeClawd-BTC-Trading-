  ...
  ({ value, onValueChange, min = 0, max = 100, step = 1, disabled, id, className, ...props }, ref) => {
    ...
    return (
      <input
        ...
        className={cn(
          'h-2 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-bitcoin-500 disabled:cursor-not-allowed disabled:opacity-50',
          '[&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-primary [&::-webkit-slider-thumb]:shadow-glow-orange',
          className,
        )}
        style={{
